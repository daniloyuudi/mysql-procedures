CREATE DEFINER=`root`@`localhost` PROCEDURE `Comparativo_Vendas_Case_Cond`()
BEGIN
DECLARE FaturamentoInicial, FaturamentoFinal, Variacao FLOAT;
    ...
    CASE
    WHEN Variacao > 0.10 THEN SELECT "Verde";
    WHEN Variacao <= 0.10 AND Variacao >= -0.10 THEN SELECT "Amarelo";
    WHEN Variacao < -0.10 THEN SELECT "Vermelho";
    END CASE;
